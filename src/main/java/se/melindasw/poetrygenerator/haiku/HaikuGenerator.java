...
public class HaikuGenerator implements HaikuGeneratorService {
  ...
  public Haiku generateHaiku(String text) throws HaikuGeneratorException {
    ...
    if (text.length() == 0) {
      throw new HaikuGeneratorException(
          "The provided text is empty. This generator only accepts a word count of 15 or more.");
    }
    ...
    while (scanner.hasNext()) {
      ...
      allWordsSortedBySyllableCountKeys
          .computeIfAbsent(value, k -> new ArrayList<String>())
          .add(wordFromTextArgument);
    }

    if (wordCount < 15) {
      throw new HaikuGeneratorException(
          "The provided text does not have enough words. This generator accepts a word count of 15 or more.");
    }
  ...
  private String stripOfDigitsAndPunctuationCharactersAndMakeLowerCase(String text) {
    ...
    return text.replaceAll("\\p{Punct}", " ").replaceAll("\\d", " ").toLowerCase();
  }

  private String getHaikuRow(int amountOfsyllablesInRow) throws HaikuGeneratorException {

    int[][] combinationsOfFiveOrSeven =
        amountOfsyllablesInRow == 5 ? combinationsOfFive : combinationsOfSeven;
